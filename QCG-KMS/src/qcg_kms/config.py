...
class Settings(BaseSettings):
    ...
    sig_backend: str = "auto"        # auto | liboqs | dilithium_py
    ...
    # How the KMS reaches Sentinel Gate for the admin dashboard. The KMS calls
    # the gateway itself and passes the result to the browser, so the gateway's
    # admin token stays on the server and the browser never talks to the gateway
    # directly. Both are optional. If they are not set, the dashboard simply
    # says monitoring is not configured rather than breaking.
    sentinel_admin_url: str | None = None    # e.g. http://127.0.0.1:8080
    sentinel_admin_token: str | None = None  # the gateway's admin bearer token
    sentinel_timeout_seconds: float = Field(default=3.0, gt=0, le=30)

    # Embedded mode: run Sentinel Gate as in-process ASGI middleware wrapped
    # around the KMS, rather than as a separate service. This is the preferred
    # topology: it puts the gateway's L7 availability protection on every KMS
    # request with no extra network hop or second process, and the dashboard
    # reads the live telemetry straight from memory in the same process. When
    # this is on, the monitor endpoint uses the in-process pipeline and the
    # external sentinel_admin_url is ignored.
    sentinel_embedded: bool = False

    # Where to keep the connection-history database that powers the dashboard's
    # date-range filters. This is separate from the key database and holds only
    # connection metadata, never key material. History older than the retention
    # window is pruned automatically. Set the path empty to disable history
    # (the live in-memory view still works).
    telemetry_history_path: str | None = None
    telemetry_history_retention_days: int = Field(default=90, gt=0, le=3650)
    ...
    @field_validator("sig_backend")
    @classmethod
    def _check_sig_backend(cls, v: str) -> str:
        if v not in {"auto", "liboqs", "dilithium_py"}:
            raise ValueError("sig_backend must be auto|liboqs|dilithium_py")
        return v
