...
class Storage:
        ...
        with self._lock, self._db:
            self._db.executescript(
                ...
                CREATE TABLE IF NOT EXISTS kms_meta (
                    id TEXT PRIMARY KEY, value TEXT NOT NULL, created_at REAL NOT NULL
                );
                """
            )
            self._migrate()
    ...
    # --- service signing identity (ML-DSA) ---------------------------------
    # The KMS holds one long-lived ML-DSA identity keypair used to sign the
    # public keys it serves, so a client can prove a recipient public key really
    # came from this KMS and was not substituted in transit. The secret half is
    # sealed under the master key exactly like a KEM private key; the database
    # alone never yields a usable signing key. The public half is served openly
    # and pinned by clients.
    _SIG_PUBLIC_ID = "service_sig_public"
    _SIG_SECRET_ID = "service_sig_secret"
    _SIG_ALG_ID = "service_sig_algorithm"

    def get_signing_identity(self) -> tuple[bytes, bytes, str] | None:
        """Return (public_key, secret_key, algorithm) or None if not yet created."""
        rows = {
            r["id"]: r["value"]
            for r in self._db.execute(
                "SELECT id, value FROM kms_meta WHERE id IN (?,?,?)",
                (self._SIG_PUBLIC_ID, self._SIG_SECRET_ID, self._SIG_ALG_ID),
            ).fetchall()
        }
        pub_b64 = rows.get(self._SIG_PUBLIC_ID)
        wrapped = rows.get(self._SIG_SECRET_ID)
        alg = rows.get(self._SIG_ALG_ID)
        if not pub_b64 or not wrapped or not alg:
            return None
        try:
            public_key = crypto._b64d(pub_b64)
            secret_key = crypto.unwrap_secret(self._master, wrapped)
        except Exception:
            # The stored signing secret cannot be unwrapped: either the master
            # key changed (e.g. an ephemeral development key across restarts) or
            # the row is corrupt. Report "no identity" so a fresh one is created,
            # rather than crashing startup. In production the master key is
            # stable, so this path is not hit for a healthy database.
            return None
        return public_key, secret_key, alg

    def set_signing_identity(self, public_key: bytes, secret_key: bytes,
                             algorithm: str) -> None:
        """Persist the service signing identity (secret sealed under the master key)."""
        pub_b64 = crypto._b64e(public_key)
        wrapped = crypto.wrap_secret(self._master, secret_key)
        ts = _now()
        with self._lock, self._db:
            self._db.executemany(
                "INSERT OR REPLACE INTO kms_meta (id, value, created_at) VALUES (?,?,?)",
                [
                    (self._SIG_PUBLIC_ID, pub_b64, ts),
                    (self._SIG_SECRET_ID, wrapped, ts),
                    (self._SIG_ALG_ID, algorithm, ts),
                ],
            )
