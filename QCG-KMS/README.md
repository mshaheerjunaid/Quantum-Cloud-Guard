...
with an **ML-KEM-1024 + AES-256-GCM** hybrid (KEM-DEM), and signs the public
keys it serves with **ML-DSA-87** (FIPS 204) so clients can verify a recipient
key genuinely came from this KMS. It is designed to run behind the
**Sentinel Gate QCG** gateway and to deploy to a VPS.
...
- **Public-key authenticity:** the KMS holds one long-lived **ML-DSA-87** identity
  keypair (secret sealed under the master key, like the KEM keys) and signs every
  public key it serves, over a canonical binding of (name, version, algorithm,
  key). The `qcg` client pins the KMS identity and verifies the signature before
  encapsulating, so a network attacker cannot substitute a key of their own. This
  is the man-in-the-middle defence, and it fails closed.
- **Dual auth:** bearer **API keys** for scripts, **username/password sessions**
  for the web UI.
- **Pluggable KEM and signature backends:** liboqs for production (matches the
  research paper's liboqs 0.15.0, which ships ML-KEM and ML-DSA) or pure-Python
  kyber-py and dilithium-py as a portable default and test backend. Auto-selection
  verifies liboqs with a real keygen and sign/verify before trusting it.
...
| GET | /api/keys/{name}/public | active public key, with ML-DSA signature |
| POST | /api/keys/{name}/rotate | new version (old versions retained) |
| DELETE | /api/keys/{name} | delete a key |
| GET | /api/signing-key | the KMS ML-DSA identity public key (for clients to pin) |
...
Pin the KMS identity once, so the client can verify the public keys it serves
(the man-in-the-middle defence):

    qcg trust                                 # pins the KMS ML-DSA-87 signing key
    qcg trust --show                          # show the pinned fingerprint

Verify the printed fingerprint out-of-band with your KMS administrator (it is
shown in the admin console under Public-Key Authenticity). After pinning, every
`qcg encrypt` verifies the recipient key's signature and refuses to encrypt to a
substituted key.
